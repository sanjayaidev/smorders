 ...
 */
async function matchItemsWithAI(text, products) {
...
async function aiFallbackReply(text, products, settings) {
  const menuForPrompt = products.map((p) => ({
    name: p.name?.en || Object.values(p.name || {})[0] || p.slug,
    price: p.price,
    currency: p.currency,
  ...
  const messages = [
    {
      role: "system",
      content: `You are a friendly WhatsApp ordering assistant for a food business. Answer the customer's question briefly (2-3 sentences max, WhatsApp style, no markdown). If they seem to want to order food, tell them to reply "order" or use the menu button. Reply with ONLY this JSON: {"reply": "<text>"}.
...
${JSON.stringify(menuForPrompt)}`,
  ...
  return `Sorry, I didn't quite catch that. Reply "${settings.order_button_label}" to place an order, or ask me anything else!`;
}
...
export async function handleIncomingMessage({ phoneNumber, profileName, text, buttonId, waMessageId }) {
  ...
  switch (conversation.stage) {
    ...
    default: {
      ...
      const products = await getActiveMenu();
      const answer = await aiFallbackReply(input, products, settings);
      await reply(answer);
    }
  }
}
