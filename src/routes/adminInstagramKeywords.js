...
/** GET /api/admin/instagram/keywords */
...
/**
 * POST /api/admin/instagram/keywords
...
/**
 * PATCH /api/admin/instagram/keywords/:id
...
/** DELETE /api/admin/instagram/keywords/:id */
router.delete("/:id", async (req, res, next) => {
  try {
    const { error } = await supabase.from("ig_keywords").delete().eq("id", req.params.id);
    if (error) throw error;
    res.status(204).end();
  } catch (err) {
    next(err);
